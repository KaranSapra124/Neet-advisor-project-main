        ...
        <>
            <Container className={' bg-gray-100/50'}>
                <div className='max-w-screen-md mx-auto'>
                    <h1 className='text-primary-color text-4xl font-bold text-center'>Gems Of <span className='uppercase text-yellow-500 font-extrabold'>neet advisor</span></h1>
                    <p className='text-gray-700 font-semibold py-2 text-xs text-center'>Vipin Bansal and Vivek Singh , two persons who dedicated their lives to transforming <strong>NEET</strong> counseling, inspiring countless students to achieve their dreams with unwavering support and expertise.</p>
                    <div className="flex flex-wrap justify-evenly items-center">
                        {
                            teamMemberData?.map((elem, index) => {
                                return <TeamMember key={index} imageUrl={elem?.imageUrl} name={elem?.name} socialLinks={elem?.socialLinks} title={elem?.title} />
                            })
                        }
    ...
    return (
        ...
        >
            <div className="our-team rounded bg-gray-100/50 p-12">
                ...
                {/* Social Links */}
                <ul className={`social-links ${isHovered ? 'social-links-visible' : ''}`}>
