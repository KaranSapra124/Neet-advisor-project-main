...
const Outcome = () => {
    ...
    return (
        <ScrollAnimation className='relative' animateIn='backInRight' animateOnce={true} duration={1.5}>
            <Container>
                <h1 className='text-primary-color font-bold text-4xl text-center'>Webinar <span className='text-yellow-500 font-extrabold'>Outcomes</span></h1>
                <p className='text-gray-700 text-center font-semibold py-2'>Our sessions are designed to drive your confidence and empower your decisions for guaranteed success.  </p>
...
const OutcomeCard = ({ className, title, imageUrl, description }) => (
    <div className={`bg-white   ${className}     shadow-md cursor-pointer m-4 rounded-lg w-72 overflow-hidden transform transition duration-500 hover:scale-110 hover:shadow-2xl`}>

        <img src={imageUrl} alt={description} className="h-48   w-full p-2 rounded-xl object-cover" />
        {/* <div className='absolute shadow-xl h-[2rem]'></div> */}
        <div className="px-2  py-2">
            <h2 className="text-md font-bold mb-3 text-primary-color">{title}</h2>
